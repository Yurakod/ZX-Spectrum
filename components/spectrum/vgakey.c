...
void menu_keyboard_configure_update(odroid_ui_entry *entry) {
    sprintf(entry->text, "%-9s", "keyboard setup");
}

void menu_load_game(odroid_ui_entry *entry) {
    sprintf(entry->text, "%-9s", "Load Game");
}

void menu_save_game(odroid_ui_entry *entry) {
    sprintf(entry->text, "%-9s", "Save Game");
}
...
odroid_ui_func_toggle_rc menu_load_game_toggle(odroid_ui_entry *entry, odroid_gamepad_state *joystick) {
    if (!joystick->values[ODROID_INPUT_A]) return ODROID_UI_FUNC_TOGGLE_RC_NOTHING;
    odroid_display_unlock();
    odroid_ui_wait_for_key(ODROID_INPUT_A, false);
    choose_file();
...
odroid_ui_func_toggle_rc menu_save_game_toggle(odroid_ui_entry *entry, odroid_gamepad_state *joystick) {
    if (!joystick->values[ODROID_INPUT_A]) return ODROID_UI_FUNC_TOGGLE_RC_NOTHING;
    odroid_display_unlock();
    odroid_ui_wait_for_key(ODROID_INPUT_A, false);
    save();
...
void menu_spectrum_init(odroid_ui_window *window) {
    ...
    odroid_ui_create_entry(window, &menu_load_game, &menu_load_game_toggle);
    odroid_ui_create_entry(window, &menu_save_game, &menu_save_game_toggle);	
}
